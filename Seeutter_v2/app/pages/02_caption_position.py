...
st.caption("실제 영상 프레임으로 네 가지 위치를 비교하세요.")

segments = caption_segments(artifact_dir)
if not segments:
    st.warning("자막 데이터가 아직 없습니다. 화자명 페이지를 먼저 완료해주세요.")
    if st.button("화자명 페이지"):
...
try:
    ...
    st.error(f"미리보기를 만들지 못했습니다: {exc}")
    st.stop()
...
with preview_column:
    st.image(
        ...
        caption="최종 영상과 같은 위치 프리셋으로 표시한 미리보기",
        use_container_width=True,
    )

back_col, _spacer, next_col = st.columns((1, 7.5, 1.3))
with back_col:
    if st.button("화자명 다시 정하기"):
        st.switch_page("pages/01_speaker_names.py")
with next_col:
